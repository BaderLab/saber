...
def test_process_args_no_cli_args(dummy_config):
...
def test_config_attributes_with_cli_args(dummy_config_cli_args):
    """Asserts that the class attributes of a config.Config object are of the expected value/type
    after object initialization, taking into account command line arguments, which take precedence
    over config arguments.
    """
...
def test_resolve_filepath(dummy_config):
    ...
    filepath_none_cli_args_none_expected = \
        resource_filename(config.__name__, constants.CONFIG_FILENAME)
    filepath_none_cli_args_none_actual = dummy_config._resolve_filepath(filepath=None, cli_args={})
    ...
    filepath_cli_args_actual = dummy_config._resolve_filepath(filepath=filepath_cli_args_expected,
                                                              cli_args=dummy_cli_args)
